...
def replace_elem(lst, index, elem):
    ...
    return [(lambda : lst[i] if i!=index else elem )() for i in range(len(lst))]
...
def put_piece(board, max_rows, column, player):
    ...
    if row==-1:
        return (row,board)
    board1=replace_elem(board[row],column,player)
    board=replace_elem(board,row,board1)
    return (row,board)
