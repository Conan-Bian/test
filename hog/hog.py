...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def f(s0,s1):
        if who==0:
            ...
            s=s1
            w=1
        if s - previous_score>previous_high :
            ph=s - previous_score
            if ph==1:
                print("%d point! That's the biggest gain yet for Player %d" % (ph,w))
            else :
                print("%d points! That's the biggest gain yet for Player %d" % (ph,w))
        else :
            ph=previous_high
        return announce_highest(w,ph,s)
    return f 
    # END PROBLEM 7
...
def bacon_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    """
    # BEGIN PROBLEM 10
    if abs(opponent_score//10-opponent_score%10)+2>=margin:
        return 0
